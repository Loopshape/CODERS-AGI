#!/usr/bin/env bash
# ai.sh - AI Autonomic Synthesis Platform v36 (Ultimate Cyberspace Cognition Edition)
# The final agent build: WebGL 3D Word Bubble Visualization + Full AGI Backend.
...
<title>AGENT NEMODIAN :: Cyberspace Cognition v36.0</title>
<style>
...
#verboseOverlay{
  ...
  overflow-y:auto; background:rgba(0,0,0,0.8); color:#0f0; padding:10px;
...
#commandBar{
  ...
  background:rgba(0,0,0,0.8); border:1px solid #00ffff; border-radius:5px;
  box-shadow: 0 0 15px #00ffff; z-index:20;
}
.input-group { display: flex; gap: 5px; }
#commandInput{
  flex-grow: 1; padding:5px; background:#111; border:1px solid #f92672; color:#a6e22e; outline:none; font-family:monospace;
}
.cli-prompt { color: #a6e22e; font-weight: bold; margin-right: 5px; }
.cli-output { color: #66d9ef; }
/* Output Styles */
.cli-info { color: #66d9ef; } .cli-success { color: #a6e22e; } .cli-error { color: #f92672; }
</style>
</head>
<body>
...
<div id="commandBar">
    ...
    <div class="input-group">
        ...
        <button id="runButton" style="background:#505050; color:white; border:none; padding:5px 10px; border-radius:3px;">RUN</button>
    </div>
    <div id="loading-indicator" style="color:#fd971f; margin-top:5px; display:none;">Thinking...</div>
</div>
...
// ---------------- Agent Class (Visualization Logic) ----------------
class Agent {
  constructor(id){ this.id=id; this.pool=[]; this.memory=[]; }
  spawnToken(word, meta){
    let token={ id:'T'+Math.floor(Math.random()*10000), word:word, metaTopic:meta, orbit:150+50*(5-meta), angle:Math.random()*Math.PI*2, speed:0.002+Math.random()*0.003, lift:0, brightness:1, x:0,y:0,z:0, active:true, history:[] };
    this.pool.push(token); this.log('spawn',token); return token;
  }
  fuseTokens(tokenA,tokenB){
    if(!tokenA.active||!tokenB.active) return null;
    let newMeta = Math.max(tokenA.metaTopic, tokenB.metaTopic);
    let newToken={ id:'F'+Math.floor(Math.random()*10000), word:tokenA.word+'-'+tokenB.word, semantic:[...new Set([...tokenA.semantic,...tokenB.semantic])], metaTopic:newMeta, orbit:150+50*(5-newMeta), angle:(tokenA.angle+tokenB.angle)/2, speed:(tokenA.speed+tokenB.speed)/2, lift:0, brightness:1.5, x:0,y:0,z:0, active:true, history:[...tokenA.history,...tokenB.history,`Fusion by Agent ${this.id}`] };
    tokenA.active=tokenB.active=false;
    this.pool.push(newToken); this.log('fusion',tokenA,tokenB); return newToken;
  }
...
// ---------------- Visualization & Backend Bridge Logic ----------------
...
function autoEvolve(agent){
    let activeTokens = agent.pool.filter(t=>t.active);
    if(activeTokens.length>1 && Math.random()<0.01){
    ...
    if(Math.random()<0.01) agent.neuroPulse();
}

function processBackendOutput(output) {
    const outputDiv = document.getElementById('verboseOverlay');
    const lines = output.split('\n');

    AGENTS.forEach(a => a.neuroPulse("BackendLog")); // Trigger visual feedback

    lines.forEach(line => {
        const cleanLine = line.replace(/\\u001b\\[[0-9;]*m/g, '').trim();
        if (!cleanLine) return;

        let color = '#66d9ef'; // Cyan (cli-info)
        let prefix = 'LOG:';

        if (cleanLine.startsWith('--- Final Answer ---')) { color = '#a6e22e'; prefix = 'FINAL:'; }
        else if (cleanLine.startsWith('AGI Loop') || cleanLine.startsWith('🚀')) { color = '#fd971f'; prefix = 'CYCLE:'; }
        else if (cleanLine.includes('ERROR') || cleanLine.includes('❌') || cleanLine.includes('FAILURE')) { color = '#f92672'; prefix = 'ERROR:'; }
        else if (cleanLine.startsWith('✅')) { color = '#a6e22e'; prefix = 'SUCCESS:'; }
        else if (cleanLine.startsWith('⚠️')) { color = '#fd971f'; prefix = 'WARN:'; }

        const logLine = document.createElement('div');
        logLine.textContent = `${prefix} ${cleanLine}`;
...
async function executeCommand(cmd) {
    ...
    // Log user command to the verbose overlay
    const promptLine = document.createElement('div');
    promptLine.innerHTML = `<span class="cli-prompt">CYBER></span> <span class="cli-output">${cmd}</span>`;
    ...
    try {
        const response = await fetch('/api/command', {
            ...
            body: JSON.stringify({ command: 'ai ' + cmd }) // Prefix with 'ai' for the Bash agent
        });
        const result = await response.json();
        
        processBackendOutput(result.output);

    } catch (e) {
        processBackendOutput(`❌ NETWORK ERROR: ${e.message}`);
    } finally {
...
function animate(){
  ...
  AGENTS.forEach(agent=>{
    ...
    agent.pool.forEach(t=>{
      ...
      t.brightness = Math.max(1.0, t.brightness - 0.015); // Decay
      ...
      const red = Math.min(255, 102 + t.metaTopic * 20);
      const green = Math.min(255, 166 + (6 - t.metaTopic) * 15);
...
commandInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') executeCommand(commandInput.value.trim()); });
runButton.addEventListener('click', () => executeCommand(commandInput.value.trim()));
